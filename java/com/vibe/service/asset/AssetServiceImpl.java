...
public class AssetServiceImpl implements AssetService {
	...
	public List<AssetVo> findAllDevice() {
		 ...
		 * null) { for (AssetVo assetVo : findDevice) { Device space = new
		 * Device(); space.setId(assetVo.getId());
		 * space.setCaption(assetVo.getCaption());
		 * space.setVendor(assetVo.getVendor()); String enabing_date =
		 * assetVo.getEnabing_date(); if (enabing_date != null &&
		 * !enabing_date.equals("")) {
		 * space.setEnabing_date(LocalDate.parse(enabing_date,
		 ...
		 * space.setEnabing_date(LocalDate.parse(enabing_date,
		 * DateTimeFormatter.ofPattern("yyyy-MM-dd"))); } dList.add(space); } }
		 */
		return findDevice;
	}
