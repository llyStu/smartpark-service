...
public class ToHtml {
    ...
    private Map<String, String> cell_merged_print = new HashMap<String, String>();// String1 =
    // (x,y),String2=rowspan,colspan
    // private int rowspan;
...
//			"dashed 1pt", BORDER_DASHED, "dashed 1pt", BORDER_DOTTED, "dotted 1pt", BORDER_DOUBLE, "double 1pt",
//			BORDER_HAIR, "solid 1px", BORDER_MEDIUM, "solid 1pt", BORDER_MEDIUM_DASH_DOT, "dashed 1pt",
//			BORDER_MEDIUM_DASH_DOT_DOT, "dashed 1pt", BORDER_MEDIUM_DASHED, "dashed 1pt", BORDER_NONE, "none",
//			BORDER_SLANTED_DASH_DOT, "dashed 1pt", BORDER_THICK, "solid 1pt", BORDER_THIN, "dashed 1pt");
    private static final Map<BorderStyle, String> BORDER = mapFor(
            ...
            BorderStyle.DOUBLE, "double 1pt",
            BorderStyle.HAIR, "solid 1px",
            BorderStyle.MEDIUM, "solid 1pt",
            BorderStyle.MEDIUM_DASH_DOT, "dashed 1pt",
//			BorderStyle.MEDIUM_DASH_DOT_DOTC, "dashed 1pt", 
            BorderStyle.MEDIUM_DASH_DOT_DOT, "dashed 1pt",
            BorderStyle.MEDIUM_DASHED, "dashed 1pt",
            BorderStyle.NONE, "none",
            BorderStyle.SLANTED_DASH_DOT, "dashed 1pt",
            BorderStyle.THICK, "solid 1pt",
    ...
    private void borderStyles(CellStyle style) {
        BorderStyle bs = style.getBorderLeftEnum();
        if (bs != BorderStyle.NONE) {
            out.format("  border-left: %s %s;%n", BORDER.get(bs), parseColor(style.getLeftBorderColor()));
        }
        bs = style.getBorderRightEnum();
        if (bs != BorderStyle.NONE) {
            out.format("  border-right: %s %s;%n", BORDER.get(bs), parseColor(style.getRightBorderColor()));
        }
        bs = style.getBorderTopEnum();
        if (bs != BorderStyle.NONE) {
            out.format("  border-top: %s %s;%n", BORDER.get(bs), parseColor(style.getTopBorderColor()));
        }
        bs = style.getBorderBottomEnum();
        if (bs != BorderStyle.NONE) {
            out.format("  border-bottom: %s %s;%n", BORDER.get(bs), parseColor(style.getBottomBorderColor()));
        }
    }

    private String parseColor(short color) {
        return String.format("#%s%s%s",
                Integer.toHexString(color >> 8 & 0x0f),
                Integer.toHexString(color >> 4 & 0x0f),
                Integer.toHexString(color & 0x0f));
    }
    ...
    public void printSheet(Sheet sheet) {
        ...
        out.format(
                "<div align=\"center\"><table class=%s cellspacing=\"0\" cellpadding=\"0\" style=\"word-break:break-all;width:"
                        + width + "px \">%n",
                DEFAULTS_CLASS);
        printCols(sheet);
    ...
    /*
     * private void printColumnHeads() { out.format("<thead>%n");
     * out.format("  <tr class=%s>%n", COL_HEAD_CLASS);
     * out.format("    <th class=%s>◊</th>%n", COL_HEAD_CLASS); //noinspection
     * UnusedDeclaration StringBuilder colName = new StringBuilder(); for (int i =
     * firstColumn; i < endColumn; i++) { colName.setLength(0); int cnum = i; do {
     * colName.insert(0, (char) ('A' + cnum % 26)); cnum /= 26; } while (cnum > 0);
     * out.format("    <th class=%s>%s</th>%n", COL_HEAD_CLASS, colName); }
     * out.format("  </tr>%n"); out.format("</thead>%n"); }
     */
        ...
        for (int num = sheet.getFirstRowNum(); num <= sheet.getLastRowNum(); num++) {
            ...
            for (int j = firstColumn; j < endColumn; j++) {
                ...
                if (shouldPrint(cell)) {
                    ...
                    try {
                        ...
                        if (rich.hasFormatting()) {
                            ...
                            for (CTRElt ct : rich.getCTRst().getRList()) {
                                ...
                                contents.append(
                                        "<font style=\"" + fontStyleDetail(font) + " \">" + ct.getT() + "</font>");
                            }
                        ...
                        if (style.getDataFormatString() != null) {
                            ...
                            cf = CellFormat.getInstance("General");
                        }
                    ...
                    if (content.equals(""))
                        content = " ";
                    out.format("    <td class=%s %s>%s</td>%n", styleName(style), attrs,
                            content.replaceAll("\\n", "<br/>").replaceAll(" ", "&nbsp;"));
                }
            }
            out.format("  </tr>%n");
    ...
    private String tagStyle(Cell cell, CellStyle style) {
        ...
        // 调整宽度
        // String width =
        // cell.getSheet().getColumnWidthInPixels(cell.getColumnIndex())+"px;";
        Font font = wb.getFontAt(style.getFontIndex());
