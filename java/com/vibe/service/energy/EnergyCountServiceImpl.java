...
//				num = ScaleUtil.getRate(((double) Math.round(avgNum * 100) / 100)*1.182);
				...
				if(list != null && !list.isEmpty()) {
					...
					rowList.add(num);
					bgm:for (int ls=2;ls < floorList.get(0).size();ls++) {
...
//		int floor = base.getFloor();
//		int catalog = base.getCatalog();
//		int type = base.getTimeType();//周  年月日  0.1.2.3
//		int week = base.getWeek();//0 表示周一，1 表示周二，……，6 表示周日  
//		Calendar cal = Calendar.getInstance();//获取当年年份
//		String yearNow = cal.get(Calendar.DAY)+"";
//		String time="";
//		switch (type) {
//		case 1:
//			time = yearNow+"-01-01";//年
//			break;
//		case 2:
//			time = yearNow+"-%-01";//月
//			break;
...
//		SimpleDateFormat df1 = new SimpleDateFormat("yyyy-MM-dd 00:00:00");//设置日期格式
	...
	public Map<String,Object> getLastNumber(int page, int size, int catalog) {
		...
		switch (catalog){
			case 34: catalog = 1002;
			break;
			case 37: catalog = 4000;
			break;
		}
		PageHelper.startPage(page, size);
		List<BaseNumber> baseList = energyCountMapper.getLastNumber(catalog,time);
